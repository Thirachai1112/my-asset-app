...
import type { Repair, SparePart, User } from '@/types'
...
export default function RepairManageTable() {
  ...
  const [partsUsage, setPartsUsage] = useState<{ part_id: string; quantity: number }[]>([])
  ...
  const openManageModal = (repair: Repair) => {
    setEditingRepair(repair)
    setStatus(repair.status || 'Pending')
    setTechnicianName(repair.technician_name || '')
    setTechnicianId(repair.technician_id ? String(repair.technician_id) : '')
  ...
  return (
    <div className="p-6">
      <div className="mb-6 flex justify-between items-center">
        ...
        <div className="max-w-xs w-full">
          <SearchInput
            ...
            placeholder="ค้นหาชื่อผู้แจ้ง,เลขทรัพย์สิน..."
          />
        </div>
      </div>
              ...
              <tr key={repair.id} className="hover:bg-slate-50/50 transition-colors text-sm">
                ...
                <td className="p-4 text-center">
                  <span className={`px-3 py-1 rounded-full text-[10px] font-bold border ${
                    REPAIR_STATUS_OPTIONS.find(s => s.value === repair.status)?.color
                  }`}>
      ...
      <Modal
        ...
        title="🛠️ บันทึกการดำเนินการซ่อม"
        darkHeader
        maxWidth="max-w-xl"
      >
        <form onSubmit={handleUpdate} className="p-6 space-y-6 overflow-y-auto">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-xs font-bold uppercase text-slate-500 mb-1.5">พนักงานผู้ตรวจสอบ</label>
              <select
                value={technicianId}
